...
double StyleManager::hctHue() const
{
    Hct hct(m_seedColor.rgba());
    return hct.get_hue();
}

double StyleManager::hctChroma() const
{
    Hct hct(m_seedColor.rgba());
    return hct.get_chroma();
}

double StyleManager::hctTone() const
{
    Hct hct(m_seedColor.rgba());
    return hct.get_tone();
}
...
void StyleManager::setSeedColorHct(double hue, double chroma, double tone)
{
    Hct hct(hue, chroma, tone);
    Argb argb = hct.ToInt();
    QColor color(QColor::fromRgba(argb));
    
    // Convert ARGB to QColor properly (handle alpha if needed, but HCT is usually opaque)
    // Hct.ToInt() returns ARGB as uint32.
    // QColor construction from uint32 might be different depending on endianness or format.
    // Let's ensure it's correct.
    // Argb is basically uint32_t.
    // QColor::fromRgba() expects 0xAARRGGBB.
