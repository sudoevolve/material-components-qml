...
Window {
    ...
    Connections {
        ...
        function onFileChanged(path) {
            ...
            // Only handle if it's a file URL
            if (desktopWindow.widgetSource.indexOf("file:///") === 0) {
                var changed = path.replace(/\\/g, "/")
                var current = desktopWindow.widgetSource.replace("file:///", "").replace(/\\/g, "/")
                ...
                // Check for match (ignoring potential existing query params in current)
                if (current.indexOf("?") !== -1) {
                    current = current.substring(0, current.indexOf("?"))
                }
                
                console.log("DesktopWidgetWindow: Comparing", changed, "with", current)

                if (changed.toLowerCase() === current.toLowerCase()) {
                    console.log("Reloading desktop widget MATCHED:", changed)
                    reloadTimer.restart()
                }
            }
        }
    ...
    Timer {
        ...
        interval: 50
        onTriggered: {
            // Clean base source to append new timestamp
            var baseSource = desktopWindow.widgetSource
            if (baseSource.indexOf("?t=") !== -1) {
                baseSource = baseSource.substring(0, baseSource.indexOf("?t="))
            }
            loader.source = baseSource + "?t=" + Date.now()
            loader.active = true
        }
    }
